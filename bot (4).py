...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    /start — crash-proof. Sends welcome message + full user menu.
    Animated emoji in message text (tg-emoji tags render in Telegram).
    Buttons use plain text labels for maximum compatibility.
    """
    ...
    # Build menu — always works, no fancy fields that can be rejected
    kb = main_menu_kb()

    async def _do_send(parse_mode="HTML", markup=None):
        text_to_send = msg if parse_mode else re.sub(r'<[^>]+>', '', msg)
        kwargs = dict(disable_web_page_preview=True)
        if parse_mode:
            kwargs['parse_mode'] = parse_mode
        if markup is not None:
            kwargs['reply_markup'] = markup
        return await update.message._orig_reply_text(text_to_send, **kwargs) if hasattr(update.message, '_orig_reply_text') else await _orig_message_reply_text(update.message, text_to_send, **kwargs)

    # Attempt 1: Full HTML + menu
    try:
        await _orig_message_reply_text(
            update.message, msg,
            reply_markup=kb,
            parse_mode="HTML",
        ...
        logger.warning(f"cmd_start attempt 1 failed: {e}")

    # Attempt 2: Strip icon/style from buttons and retry
    try:
        safe_kb = _strip_button_extras(kb)
        await _orig_message_reply_text(
            update.message, msg,
        ...
        logger.warning(f"cmd_start attempt 2 failed: {e}")

    # Attempt 3: Plain text + stripped buttons
    try:
        plain_msg = re.sub(r'<[^>]+>', '', msg)
        safe_kb   = _strip_button_extras(kb)
        await _orig_message_reply_text(
            update.message, plain_msg,
            reply_markup=safe_kb
        )
        return
    except Exception as e:
        logger.warning(f"cmd_start attempt 3 failed: {e}")

    # Attempt 4: Bare minimum — text only, no markup
    try:
        plain_msg = re.sub(r'<[^>]+>', '', msg)
        await _orig_message_reply_text(update.message, plain_msg)
    except Exception as e:
        logger.error(f"cmd_start all attempts failed: {e}")
