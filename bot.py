...
def admin_list_kb(admins: list) -> InlineKeyboardMarkup:
    ...
    for aid in admins:
        ...
        kb.append([
            InlineKeyboardButton(label, callback_data="ignore", style="primary", icon_custom_emoji_id=icon),
            InlineKeyboardButton("Remove", callback_data=f"rm_admin_{aid}", style="danger", icon_custom_emoji_id=_UI["trash"][0])
        ])
    kb.append([InlineKeyboardButton("Add Admin", callback_data="add_admin_prompt", style="success", icon_custom_emoji_id=_UI["bell"][0])])
    kb.append([InlineKeyboardButton("Back", callback_data="admin_home", style="primary", icon_custom_emoji_id=_UI["back"][0])])
    return InlineKeyboardMarkup(kb)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="virtual_web":
        await query.answer("This flow moved to the website dashboard.", show_alert=True)
        await query.edit_message_text(
            f"{ui('info')} <b>Virtual Number Web moved</b>\n{D}\nUse the website dashboard for the public temp-number flow.\n\n{ui('rocket')} Bot menu now keeps only the classic Telegram assignment flow.",
            reply_markup={"inline_keyboard": [[btn("Main Menu", cbd="main_menu", style="primary", icon=_UI["back"][0])]]},
            parse_mode="HTML",
        )
        return
