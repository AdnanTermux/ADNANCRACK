...
def InlineKeyboardButton(text='', *, callback_data=None, url=None,
    ...
    if pay:                                          kw2['pay'] = pay
    if icon_custom_emoji_id:                         kw2['icon_custom_emoji_id'] = icon_custom_emoji_id
...
_UI = {
    ...
    # ── Additional keys used throughout codebase ──────────────────────────
...
def otp_keyboard(otp: Optional[str], full_msg: str = "",
    ...
    if for_group:
        if clean:
            rows.append([btn(f"🔐 Tap to Copy OTP", copy=clean,
    ...
    # T13: ROYAL PREMIUM 👑
        ...
        if dev_url:   r1.append(_link("⚙️ Dev",     "gear",   dev_url))
        if r1: rows.append(r1)
        r2 = []
    ...
    # T14: HOVER LUXURY 💎
        ...
        if r1: rows.append(r1)
        r2 = []
        if dev_url:     r2.append(_link("⚙️ Developer", "gear",   dev_url,     "primary"))
        if support_url: r2.append(_link("🛡 Support",   "shield", support_url, "danger"))
        if r2: rows.append(r2)
...
def main_menu_compact_kb() -> dict:
    ...
    row4.append(btn("⚙️  Settings", cbd="user_settings", style="primary", icon=_UI["settings"][0]))
    rows.append(row4)
...
def admin_main_kb(perms: list, is_sup: bool) -> dict:
    ...
    r = []
    if is_sup: r.append(btn(f"{ui('key')}  API Tokens",  cbd="admin_api_tokens", style="success"))
    if r: rows.append(r)
