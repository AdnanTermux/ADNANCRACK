...
def btn(text: str, cbd: str = None, url: str = None,
        style: str = None, copy: str = None, icon: str = None) -> dict:
    """
    Build an InlineKeyboardButton dict with animated emoji support.

    icon  = _UI['name'][0]            → icon_custom_emoji_id (animated emoji on button)
    style = 'primary'|'success'|'danger'  → colored button (Bot API 9.4+)
    HTML/tg-emoji tags stripped from text automatically.
    """
    resolver = globals().get("_resolve_button_payload")
    clean_text, icon = resolver(text, icon) if callable(resolver) else (_btn_clean(text), icon)
    b: dict = {'text': clean_text}
    if cbd:   b['callback_data'] = cbd
    if url:   b['url'] = url
    if copy:  b['copy_text'] = {'text': copy}
    if style: b['style'] = style
    if icon:  b['icon_custom_emoji_id'] = str(icon)
    return b
...
def otp_keyboard(otp: Optional[str], full_msg: str = "",
    ...
    # ─── shorthand helpers ─────────────────────────────────────────
    def _copy(label: str, iname: str) -> dict:
        return btn(label, copy=clean, icon=_UI[iname][0], style="success")
    ...
    # ─── GROUP mode: OTP visible in copy button, plus links ────────
    if for_group:
        if clean:
            rows.append([btn("Tap to Copy OTP", copy=clean,
                             icon=_UI["lock"][0], style="success")])
        lr = []
    ...
    # T0: CLASSIC ⭐
    if t == 0:
        if clean:    rows.append([_copy("Copy OTP", "lock")])
    ...
    # T1: MINIMAL
        ...
        if clean: rows.append([_copy("Copy OTP", "key")])
    ...
    # T3: ELECTRIC ⚡
        ...
        if clean: rows.append([_copy("Copy OTP", "bolt")])
        lr = []
    ...
    # T5: PREMIUM 💎
        ...
        if clean:    rows.append([_copy("Copy OTP", "diamond")])
    ...
    # T7: BUSINESS 💼
        ...
        if clean: rows.append([_copy("Copy OTP", "chart")])
        lr = []
        if panel_url:   lr.append(_link("Bot", "robot", panel_url, "primary"))
    ...
    # T8: SOCIAL 🌐
        ...
        if clean: rows.append([_copy("Copy OTP", "people")])
        r1 = []
    ...
    # T9: DELUXE 🌟
        ...
        if clean:    rows.append([_copy("Copy OTP", "star")])
        if full_msg: rows.append([_msg("Full Text", "notepad")])
        r1 = []
    ...
    # T10: CLASSIC ELEGANCE 📝
        ...
        if clean:    rows.append([_copy("Copy OTP", "notepad")])
        if full_msg: rows.append([_msg("Full Message", "document")])
        lr = []
        if panel_url: lr.append(_link("Get Numbers", "robot", panel_url))
    ...
    # T12: FOCUS 🎯
        ...
        if clean: rows.append([_copy("Copy OTP", "focus")])
        lr = []
        if panel_url: lr.append(_link("Bot", "robot", panel_url))
        if info_url:  lr.append(_link("Channel", "announce", info_url, "success"))
        if lr: rows.append(lr)
    ...
    # T13: ROYAL PREMIUM 👑
        ...
        if clean:    rows.append([_copy("Copy OTP", "crown")])
        if full_msg: rows.append([_msg("Full Text", "notepad")])
        r1 = []
...
def main_menu_compact_kb() -> dict:
    ...
    # ✅ Row 1: Classic flow + premium virtual web flow
    rows.append([
        btn("Get Number", cbd="buy_menu", style="success", icon=_UI["fire"][0]),
        btn("Virtual Number Web", cbd="virtual_web", style="primary", icon=_UI["desktop"][0]),
    ])
...
def vweb_countries_kb(countries: list, page: int = 1) -> dict:
...
def vweb_services_kb(country: str, services: list, page: int = 1) -> dict:
...
def waiting_kb(prefix=None, service=None) -> dict:
    ...
    rows = [
        [btn("Unlimited Access" if DEFAULT_ASSIGN_LIMIT == -1 else f"Limit: {DEFAULT_ASSIGN_LIMIT}",
...
async def show_buy_menu(query, uid: int):
    ...
    unlimited_msg = (
        f"\n\n{emoji('lock', uid)} <b>Unlimited Numbers Access Active</b>"
        if DEFAULT_ASSIGN_LIMIT == -1 else ""
    )
    if not svcs:
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [[btn("Back", cbd="main_menu", style="primary", icon=_UI["back"][0])]]},
            parse_mode="HTML",
        )
        return
    await query.edit_message_text(
        f"{ui('phone')} <b>Select Service</b>{unlimited_msg}\n{D}",
...
async def show_virtual_country_menu(query, context: ContextTypes.DEFAULT_TYPE,
    ...
    if not countries:
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [[btn("Main Menu", cbd="main_menu", style="primary", icon=_UI["rocket"][0])]]},
            parse_mode="HTML",
        )
        return
    parts = [
...
def admin_main_kb(perms: list, is_sup: bool) -> dict:
    ...
    # API TOKENS (visible to super admins)
    r = []
    if is_sup: r.append(btn("API Tokens", cbd="admin_api_tokens", style="success", icon=_UI["key"][0]))
    if r: rows.append(r)
...
def confirm_del_panel_kb() -> dict:
    return {"inline_keyboard": [[
        btn("Yes, Delete", cbd="p_del_confirm", style="danger", icon=_UI["check"][0]),
        btn("Cancel", cbd="admin_panel_manager", style="primary", icon=_UI["cancel"][0]),
    ]]}

def confirm_block_kb() -> dict:
    return {"inline_keyboard": [[
        btn("Yes, Block", cbd="block_yes", style="success", icon=_UI["check"][0]),
...
def admin_settings_kb() -> dict:
    ...
    return {"inline_keyboard": [
        [btn(f"Limit: {limit_label}", cbd="set_limit", style="success", icon=_UI["lock"][0])],
...
def confirm_kb(action: str) -> dict:
    return {"inline_keyboard": [[
        btn("Confirm", cbd=f"confirm_{action}", style="success", icon=_UI["check"][0]),
...
async def do_sms_hit(bot_app,db_obj,otp_code,msg_body,service_name,panel_name,num_raw,session):
    ...
    if otp_code:
        ...
        limit=await get_user_assign_count(user_id)
        ...
        if active and msg_id:
            try:
                ...
                if flow.get("mode") == "virtual":
                    text = await build_virtual_number_text_from_active(
                        user_id,
                        active,
                        prefix=pfx,
                        latest_phone=num_raw,
                        latest_otp=clean,
                        latest_message=msg_body,
                    )
                else:
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    lim_badge = (f"{ui('lock')} <b>UNLIMITED ACCESS</b>"
                 if DEFAULT_ASSIGN_LIMIT == -1
                 else f"{ui('copy')} <b>Limit: {DEFAULT_ASSIGN_LIMIT}</b>")
    ...
    if IS_CHILD_BOT:
        ...
        parts.append(
            ...
            f"{ui('desktop')} Virtual Number Web  |  Country-first live inbox\n\n"
    ...
    if context.user_data.get("awaiting_prefix"):
        ...
        if user_text.lower()=="off":
            ...
            if cnt>0:
                ...
                if phones:
                    active=await db.get_active_numbers(uid)
                    flow = get_number_flow_state(uid)
                    svc=(active[0].category.split(" - ")[1] if active and " - " in active[0].category else cat)
                    if flow.get("mode") == "virtual":
                        text = await build_virtual_number_text_from_active(uid, active, prefix=user_text)
                    else:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data=="virtual_web":
        await query.answer()
        logger.info(f"{emoji('desktop', uid)} Virtual Number Web opened | UID: {uid}")
        await show_virtual_country_menu(query, context, uid, page=1)
        return
    ...
    if data.startswith("vweb_country_page_"):
        ...
        await show_virtual_country_menu(query, context, uid, page=page)
        return
    ...
    if data.startswith("vweb_country_pick_"):
        ...
        if idx < 0 or idx >= len(countries):
            await show_virtual_country_menu(query, context, uid, page=1)
            return
        flag, country = countries[idx]
        await show_virtual_service_menu(query, context, uid, country=country, page=1)
        return

    if data.startswith("vweb_service_page_"):
        await query.answer()
        page = max(1, int(data.rsplit("_", 1)[1]))
        await show_virtual_service_menu(query, context, uid, page=page)
        return
    ...
    if data=="change_service_menu":
        ...
        flow = get_number_flow_state(uid)
        if flow.get("mode") == "virtual":
            await show_virtual_service_menu(query, context, uid, country=flow.get("country"), page=1)
        else:
            await show_buy_menu(query, uid)
        return
    ...
    if data=="skip_next":
        ...
        if flow.get("mode") == "virtual":
            ...
            flag = flow.get("flag") or "🌍"
            if not country or not service:
                await show_virtual_country_menu(query, context, uid, page=1)
                return
            ok, _ = await db.release_number(uid)
            if ok:
                await db.request_numbers(uid, f"{flag} {country} - {service}", count=1)
                active = await db.get_active_numbers(uid)
                if active:
                    pfx = await db.get_user_prefix(uid)
                    text = await build_virtual_number_text_from_active(uid, active, prefix=pfx)
                    ...
                    await show_virtual_service_menu(
                        query,
                        context,
                        uid,
                        country=country,
                        page=1,
                        notice=styled_warning(
                            "Out Of Stock",
                            f"No fresh numbers are available right now for <b>{html.escape(service)}</b> in <b>{html.escape(country)}</b>.",
                        ),
                    )
            else:
                await query.answer("🚫 No active number assigned.", show_alert=True)
            return
    ...
    if data=="block_no":
        ...
        if active:
            ...
            if flow.get("mode") == "virtual":
                await query.edit_message_text(
                    await build_virtual_number_text_from_active(uid, active, prefix=pfx),
                    reply_markup=waiting_kb(pfx, service=flow.get("service")),
                    parse_mode="HTML",
                )
                return
            svc=active[0].category.split(" - ")[1] if " - " in active[0].category else active[0].category
        ...
        if ok and cat:
            ...
            if flow.get("mode") == "virtual":
                await show_virtual_service_menu(
                    ...
                    country=flow.get("country"),
                    page=1,
                    notice=styled_success(
                        "Number Blocked",
                        "Pick another service or route to continue with a fresh live inbox.",
                    ),
                )
                return
    ...
    if data=="clear_all_logs":
        ...
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [[
                {"text": "✅  Yes, Clear All", "callback_data": "confirm_clear_logs", "style": "danger"},
                {"text": "❌  Cancel", "callback_data": "admin_manage_logs", "style": "primary"},
            ]]}, parse_mode="HTML")
        return
    ...
    if data=="confirm_clear_logs":
        ...
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [[
                {"text": "🔙 Back", "callback_data": "admin_home", "style": "primary"}
            ]]}, parse_mode="HTML")
        return
    ...
    if data=="admin_system":
        ...
        await query.edit_message_text(
            ...
            reply_markup={"inline_keyboard": [
                [{"text": f"{ui('database')} DB Summary",  "callback_data": "admin_db_summary",  "style": "primary"},
                 {"text": f"{ui('chart')} OTP History",    "callback_data": "admin_otp_history", "style": "success"}],
                [{"text": f"🧹 Maintenance",               "callback_data": "admin_maintenance", "style": "danger"},
                 {"text": f"{ui('notepad')} View Logs",    "callback_data": "view_logs",         "style": "primary"}],
                [{"text": "🔙 Back",                        "callback_data": "admin_home",        "style": "primary"}],
            ]}, parse_mode="HTML")
        return
