  ...
  <section class="feed-card route-card">
    ...
    <div class="route-grid">
      <div class="route-panel">
        ...
        <p>This website view does not consume Telegram bot assignments. It simply opens a live public inbox from the available route stock.</p>
...
async function openRoute(exclude=''){
  ...
  try{
    const params=new URLSearchParams({country,service});
    if(exclude) params.set('exclude',exclude);
    const r=await fetch(`/api/public/get-number?${params.toString()}`);
    if(!r.ok) throw new Error('HTTP '+r.status);
    const d=await r.json();
    if(d.status!=='success' || !d.data) throw new Error(d.message||'No live number available');
    routeState.current=d.data;
...
function renderRouteInbox(data){
  ...
  const available=data.available_numbers ?? routeState.current?.available_numbers ?? 0;
  document.getElementById('routeUpdated').textContent=`Updated: ${fmtClock(new Date())}`;
  document.getElementById('routeMeta').textContent=`${available.toLocaleString()} live numbers in ${data.country} • ${data.service}`;
  document.getElementById('routeInbox').innerHTML=`<div class="chart-kicker">Live public inbox</div>
    ...
    <div class="inbox-sub">Random public route selected from the website inventory. New OTPs will appear below when they land on this number.</div>
    <div class="meta-row">
      <span class="meta-pill">Available ${e(String(available))}</span>
  ...
  <section class="hero">
    ...
    <div class="hero-side">
      <div class="mini">
        ...
        <strong>/api/public/otps, /api/public/stats, and /api/public/get-number/*</strong>
      </div>
      <div class="mini">
...
<pre>async function openPublicInbox(country, service, currentNumber) {
  const params = new URLSearchParams({ country, service });
  if (currentNumber) params.set('exclude', currentNumber);

  const pickRes = await fetch(`https://tempnum.net/api/public/get-number?${params}`);
  const pick = await pickRes.json();
  if (pick.status !== 'success') throw new Error(pick.message || 'Route unavailable');

  renderNumberCard(pick.data.number, pick.data.country, pick.data.service);
  renderInboxHistory(pick.data.history);

  setInterval(async () => {
    const refreshRes = await fetch(
      `https://tempnum.net/api/public/get-number/history?number=${encodeURIComponent(pick.data.number)}`
    );
