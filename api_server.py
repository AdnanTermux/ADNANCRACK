...
async def security_headers(request: Request, call_next):
    ...
    if request.url.path.startswith("/api/public/otps"):
        ...
        response.headers["Cache-Control"] = "no-store"
    if request.url.path in ("/", "/api/docs", "/create-my-bot"):
...
WEBSITE_REQUESTS_DIR = "website_bot_requests"
        ...
        async with AsyncSessionLocal() as s:
            if previous_number:
                ...
                if previous:
                    ...
                    previous.assigned_to = None
                    previous.assigned_at = None
                    previous.retention_until = None
            ...
            picked.assigned_to = PUBLIC_ASSIGN_USER_ID
            picked.assigned_at = now
            picked.retention_until = now + timedelta(seconds=PUBLIC_ASSIGN_TTL_SECONDS)
            await s.commit()
    ...
    try:
        ...
        async with AsyncSessionLocal() as s:
            current_number = await s.scalar(
                select(Number).where(Number.phone_number == normalized)
            )
...
.result-meta{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}
.meta-box{
...
@media (max-width:980px){
  .hero,.grid,.contact-grid,.result-meta,.form-grid{grid-template-columns:1fr}
}
@media (max-width:640px){
  ...
  <section class="grid">
    <article class="card">
      <div class="panel active" id="formPanel">
        ...
        <p class="section-copy">Use the same information you would send in the Telegram create-bot flow. The more accurate the details are, the faster the owner can review the request.</p>
        <form id="createBotForm" onsubmit="startVerification(event)">
          <div class="form-grid">
            ...
            <div class="field full">
              <label for="group_id">OTP group chat ID</label>
              <input id="group_id" maxlength="24" placeholder="-1001234567890">
              <div class="hint">This should be the numeric Telegram chat ID where OTP messages will be delivered.</div>
            </div>
          </div>
          <div class="actions">
...
window.onload=()=>{ loadSiteSettings(); };
...
async function submitBotRequest(method){
  ...
  steps.forEach((row,idx)=>{
    ...
    if(idx===0) row.querySelector('p').textContent='Submitting your request to the website queue';
    if(idx===1) row.querySelector('p').textContent='Routing the request toward the Telegram admin bot';
    if(idx===2) row.querySelector('p').textContent='Preparing your owner contact message';
  });
...
function renderResult(data){
  ...
  document.getElementById('resultNote').textContent=(data.approval_note || 'Contact the owner and mention your request ID.') + ' Admins can review the same request inside the Telegram bot.';
